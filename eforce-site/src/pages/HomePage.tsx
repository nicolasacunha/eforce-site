    ...
    <>
      ...
      <section style={{ background: "#0a0a0a", padding: "clamp(6rem, 15vh, 12rem) clamp(1.5rem, 6vw, 6rem)" }}>
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4" style={{ gap: "clamp(1rem, 3vw, 2rem)" }}>
          {[
            { route: `/${lang}/technology`, title: t("nav.technology"), sub: "explore nossos módulos" },
            { route: `/${lang}/dealers`,    title: t("nav.dealers"),    sub: "encontre nossos parceiros" },
            { route: `/${lang}/story`,      title: t("nav.story"),      sub: "descubra nossa história" },
            { route: `/${lang}/support`,    title: t("nav.support"),    sub: "manuais e catálogos" },
          ].map(({ route, title, sub }) => (
            <div key={route} className="cursor-pointer" onClick={() => navigate(route)}>
              <GlowCard
                customSize
                glowColor="orange"
                className="w-full"

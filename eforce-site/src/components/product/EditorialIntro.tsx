          ...
          <div className="relative">
            ...
            {images.length > 1 && (
              <div className="flex justify-center gap-4 mt-4">
                <button onClick={() => setCurrentImage((prev) => (prev - 1 + images.length) % images.length)} className="text-gray-500 hover:text-gray-900 transition-colors">← {t("product.previous")}</button>
                <span className="text-gray-400 text-sm">{currentImage + 1} / {images.length}</span>
                <button onClick={() => setCurrentImage((prev) => (prev + 1) % images.length)} className="text-gray-500 hover:text-gray-900 transition-colors">{t("product.next")} →</button>
              </div>
            )}
          </div>
        )}
